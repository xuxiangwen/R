...
lmGradientDescent <- function(X, y, num_iters, alpha=0.01, scale=TRUE, center=TRUE) {
  ...
  if (scale) theta[-1] <- sapply(1:ncol(X), function(i) theta[i+1]/sd(X[,i])) 
  if (center) {
    if (scale) {
      theta[1] <- theta[1] - sapply(1:ncol(X), function(i) theta[i+1]*mean(X[,i])/sd(X[,i]))
    } else {
      theta[1] <- theta[1] - sapply(1:ncol(X), function(i) theta[i+1]*mean(X[,i]))
    }
  } 
...
lmGradientDescent(X=X, y=y, num_iters=50, alpha=0.2, center=FALSE)
lmGradientDescent(X=X, y=y, num_iters=50, alpha=0.2, center=FALSE, scale=FALSE)
...
y<-mtcars[,1]
lmCost(X, y)
lmGradientDescent(X=X, y=y, num_iters=1000, alpha=0.001)
lmGradientDescent(X=X, y=y, num_iters=1000, alpha=0.001, isScale=TRUE)
```
